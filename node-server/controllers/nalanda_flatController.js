...
const Nalanda = require("../models/Nalanda");

// GET all flats
exports.getFlats = async (req, res) => {
  try {
    const flats = await Nalanda.find();
    res.json(flats);
  } catch (err) {
...
exports.createFlat = async (req, res) => {
  try {
    const flat = new Nalanda(req.body);
    await flat.save();
...
exports.getFlatById = async (req, res) => {
  ...
  try {
    const flat = await Nalanda.findOne({ flat_no: flatNo });
...
exports.getRoomByNumber = async (req, res) => {
  ...
  try {
    const flat = await Nalanda.findOne({ flat_no: flatNo }).populate({
...
exports.getSlotByNumber = async (req, res) => {
  ...
  try {
    const flat = await Nalanda.findOne({ flat_no: flatNo }).lean();
    if (!flat) return res.status(404).json({ error: "Flat not found" });
